#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build a deduplicated optimization backlog with an evidence-based lifecycle."""
...
HEALTH_HISTORY = ROOT / "module_health_history.json"
ACTIONS = {
...
def build_backlog(
    ...
    health_history = _load(HEALTH_HISTORY) if health_history is None else health_history
    ...
    for item in previous_items if isinstance(previous_items, list) else []:
        ...
        if module:
            previous_by_module[str(module)] = item
        else:
            key = str(item.get("dedupe_key", ""))
            if key:
                previous_by_module[key.split(":", 1)[-1]] = item
    ...
    for module, row in (trend.get("modules", {}) or {}).items():
        ...
        module = str(module)
        direction = row.get("direction")
        triggered = _triggered(row)
        key = f"quality:{module}"
        old = previous_by_module.get(module)
        verification = verify_fix(module, health_history)

        if not triggered:
            if old:
                if verification["status"] == "regressed":
                    status = "regressed"
        ...
        if old and old.get("status") == "resolved":
            status = "regressed"
        elif old and old.get("status") in {"open", "regressed"}:
            status = "open"
        elif old:
            status = "recovering"
        else:
        ...
        fingerprint = hashlib.sha256(
            f"{module}|{direction}|{round(float(row.get('error_rate') or 0.0), 4)}".encode()
        ).hexdigest()[:16]
        active.append(
            {
                "backlog_id": f"quality-{module}",
                "module": module,
                "priority": priority,
                "direction": direction,
    ...
    active.sort(
        key=lambda item: (
            item.get("status") in {"open", "regressed"},
            item.get("priority", 0),
            item.get("module", ""),
        ),
        reverse=True,
...
def main() -> None:
    ...
    health_history = _load(HEALTH_HISTORY)
    result = build_backlog(trend, previous, health_history)
