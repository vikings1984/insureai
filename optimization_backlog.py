...
def append_history(backlog:dict, history:dict|None=None)->dict:
    ...
    snaps.append({"items":[{"dedupe_key":x.get("dedupe_key"),"status":x.get("status"),"priority":x.get("priority",0)} for x in backlog.get("items",[])])
