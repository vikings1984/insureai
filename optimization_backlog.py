...
def build_backlog(trend: dict|None=None, previous: dict|None=None)->dict:
    ...
    previous_items={str(x.get("dedupe_key")):x for x in previous.get("items",[]) if isinstance(x,dict)}
    active=[]
    for module,row in (trend.get("modules",{}) or {}).items():
        if not isinstance(row,dict): continue
        direction=row.get("direction"); triggered=_triggered(row)
        key=f"{module}:{direction or 'baseline'}"
        old=previous_items.get(key)
        if not triggered:
            if old:
                status="resolved" if old.get("status") in {"open","recovering","regressed"} else old.get("status","resolved")
                active.append({**old,"status":status,"resolved_at_next_snapshot":True,"error_rate":row.get("error_rate",0.0),"error_rate_delta":row.get("error_rate_delta")})
            continue
...
def append_history(backlog:dict, history:dict|None=None)->dict:
    history=history or _load(HISTORY)
    snaps=history.get("snapshots",[]) if isinstance(history,dict) else []
    if not isinstance(snaps,list): snaps=[]
    snapshot_items=[]
    for x in backlog.get("items",[]):
        if isinstance(x,dict):
            snapshot_items.append({"dedupe_key":x.get("dedupe_key"),"status":x.get("status"),"priority":x.get("priority",0)})
    snaps.append({"items":snapshot_items})
    return {"version":1,"snapshots":snaps[-90:]}

def main()->None:
    trend=_load(TREND); previous=_load(OUTPUT); history=_load(HISTORY)
    result=build_backlog(trend,previous)
    OUTPUT.write_text(json.dumps(result,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    HISTORY.write_text(json.dumps(append_history(result,history),ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
