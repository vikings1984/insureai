...
ROOT=Path(__file__).resolve().parent; TREND=ROOT/"module_health_trend.json"; OUTPUT=ROOT/"optimization_backlog.json"
...
def build_backlog(trend:dict|None=None)->dict:
    trend=_load(TREND) if trend is None else trend; items=[]
    for module,row in (trend.get("modules",{}) or {}).items():
        if not isinstance(row,dict): continue
        direction=row.get("direction")
        if direction not in {"worsening","baseline"} and row.get("health") not in {"critical","watch"}: continue
        ...
        items.append({"backlog_id":f"quality-{module}-{fingerprint}","module":module,"priority":priority,"direction":direction,"health":row.get("health","no_signal"),"error_rate":row.get("error_rate",0.0),"error_rate_delta":row.get("error_rate_delta"),"optimization_action":ACTIONS.get(module,ACTIONS["unknown"]),"source":"module_health_trend","automation":"advisory_only","dedupe_key":f"{module}:{direction}"})
    items.sort(key=lambda x:(x["priority"],x["module"]),reverse=True)
    return {"version":1,"principle":"质量趋势只生成内部优化建议，不直接改变线上判断或创建外部任务","baseline_available":bool(trend.get("baseline_available")),"items":items[:50]}
def main()->None:
    r=build_backlog(); OUTPUT.write_text(json.dumps(r,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
