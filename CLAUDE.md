...
- **部署验证分两条职责，别混为一谈**：
  - **发布门禁** = `deploy-cloudflare.yml`，内联在 `wrangler deploy` 之后，与发布天然对齐，要求 marker **严格一致**。
  - **存活兜底** = `deployment-verification.yml`，每 6 小时独立跑，**刻意与发布链解耦**（不能加 `workflow_run`，否则部署没跑时兜底也跟着不跑）。它容忍"站点仍在旧但曾发布过的 marker 上"（记 `stale`、`exit 0`），只有不可达/内容类型异常/未知 marker 才失败。
  - 历史坑：兜底曾用 HEAD marker 强一致判定，遇 `release_manifest.py` 重新盖章的传播窗口就误报失败（近半数红灯），站点其实一直 200。已由 `status=stale` + `DEPLOYMENT_TOLERATE_STALE` 修复。判读 CI 时记住：`stale` 是传播延迟，不是宕机。
