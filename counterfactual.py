...
def build_counterfactual(data: dict) -> dict:
    ...
    for event in events:
        ...
        if not base:
            continue
        scenario = 'remove_conflict_flag'
        cf_event = _strip_conflicting_signal(event)
        cf = _decision_by_event([cf_event], temporal).get(eid, base)
        fragility = base['urgency'] != cf['urgency']
        cases.append({
            'event_id': eid,
            'scenario': scenario,
            'baseline_urgency': base['urgency'],
            'counterfactual_urgency': cf['urgency'],
            'changed': fragility,
            'note': '仅移除冲突标记；用于验证决策是否被单一信号支配。',
        })
        signal_topic = event.get('topic')
        if signal_topic:
            cf_temporal = _strip_trend(temporal, signal_topic)
            cf2 = _decision_by_event([event], cf_temporal).get(eid, base)
            cases.append({
                'event_id': eid,
                'scenario': 'remove_topic_trend_signal',
                'baseline_urgency': base['urgency'],
                'counterfactual_urgency': cf2['urgency'],
                'changed': base['urgency'] != cf2['urgency'],
                'note': '移除该主题趋势信号；用于验证现在/近期判断是否过度依赖时间趋势。',
            })
    changed = sum(1 for x in cases if x['changed'])
    return {
        'version': 1,
        'principle': '高价值结论必须在去除单一关键输入后保持合理稳健，否则进入人工复核。',
        'total_cases': len(cases),
        'fragile_cases': changed,
        'fragility_rate': round(changed / len(cases), 4) if cases else 0.0,
        'cases': cases[:500],
    }
