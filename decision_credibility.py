...
def build_credibility() -> dict:
    ...
    if quality_status != "passed":
        reasons.append(_reason("quality_not_passed", "blocked", "质量门禁未通过，不能把当前决策视为可发布。", "quality_status", quality_status, "passed"))
    if deployment_status not in {"verified", "pending"}:
        reasons.append(_reason("deployment_state_invalid", "caution", "部署状态不是受支持的 pending/verified 状态。", "deployment_status", deployment_status, ["verified", "pending"]))
    elif not deployment_verified:
        reasons.append(_reason("deployment_not_verified", "review", "质量通过不等于生产部署已验收；当前发布身份尚未得到线上验证。", "deployment_verified", deployment_verified, True))
    if jitter > 0:
        reasons.append(_reason("decision_jitter_detected", "review", "决策稳定性检测发现 jitter，需要人工复核受影响判断。", "decision_jitter_events", jitter, 0))
    if low_availability > 0:
        reasons.append(_reason("low_or_unavailable_evidence", "review", "存在低可用或不可用证据，相关判断不应自动升级为确定结论。", "low_or_unavailable_evidence", low_availability, 0))
    if macro_quality is None:
        reasons.append(_reason("macro_quality_missing", "caution", "缺少宏观质量指标，可信度不能进入 ready。", "macro_quality", None, ">= 0.95"))
    elif macro_quality < 0.95:
        reasons.append(_reason("macro_quality_below_gate", "caution", "宏观质量低于当前质量门槛。", "macro_quality", macro_quality, 0.95))
    ...
    signal_details = [
        ...
        {"signal": "deployment_verified", "actual": deployment_verified, "comparator": "==", "threshold": True, "result": deployment_verified, "source": "release_manifest.json"},
