...
def _priority(event: dict, decision: dict | None, counterfactual: dict | None = None) -> int:
    ...
    if trust == "low":
        ...
        priority += 10
    if (event.get("trust") or {}).get("conflict"):
        priority += 30
...
def _candidate_reasons(event: dict, decision: dict | None, temporal: dict | None, counterfactual: dict | None) -> list[dict]:
    ...
    if decision and decision.get("urgency") == "now" and trust.get("level") != "high":
    ...
    if signal and signal.get("phase") in {"accelerating", "forming"} and int(signal.get("current_period_count") or 0) < 3:
...
def build_review_queue(data: dict, counterfactual_cases: list[dict] | None = None) -> dict:
    ...
    for event in events:
        ...
        candidates.append({
            ...
            "decision": {"urgency": decision.get("urgency"), "action": decision.get("action")} if decision else None,
        })
