...
ACTION_WORDS = ["acquire","acquisition","buy","merger","launch","unveil","regulation","rule","fine","approval","invest","investment","expand","enter","exit","raise","funding","upgrade","downgrade","loss","claim","收购","并购","发布","推出","获批","监管","处罚","投资","扩张","退出","融资","升级","下调","理赔","试点","落地","三审","政策","改革"]
...
ACTION_BY_TYPE = {"acquisition":"acquire","regulatory":"regulate","product":"launch","capital":"invest","market_entry":"enter","rating":"rate","claims_loss":"loss","personnel":"appoint","industry_update":"update"}


def _norm(text: str) -> str:
    return re.sub(r"[^\w\u4e00-\u9fff]+", " ", re.sub(r"https?://\S+", " ", (text or "").lower()), flags=re.UNICODE).strip()
...
def _entities(item: dict) -> list[str]:
    ...
    if isinstance(tags, str): values.extend(x.strip() for x in tags.split(",") if x.strip())
...
def _event_type(item: dict) -> str:
    ...
    ranked = [(sum(1 for word in words if word in text), event_type) for event_type, words in EVENT_TYPES.items()]
...
def _signature(item: dict) -> str:
    ...
    raw = tuple(sorted(_tokens(title))[:12]) + tuple(sorted(_entities(item))[:6])
    return hashlib.sha1("|".join(raw).encode("utf-8")).hexdigest()[:12]


def _event_fingerprint(item: dict) -> str:
    event_type, anchor, topic, ts = _event_type(item), _entity_anchor(item) or "unknown-entity", item.get("research_topic") or "general", _timestamp(item)
...
def _entity_similarity(a: dict, b: dict) -> float:
    ...
    return len(ea & eb) / max(1, len(ea | eb)) if ea and eb else 0.0


def _event_similarity(a: dict, b: dict) -> float:
    ta, tb = a.get("title_zh") or a.get("title") or "", b.get("title_zh") or b.get("title") or ""
    return min(1.0, 0.55*_token_similarity(ta,tb) + 0.30*_entity_similarity(a,b) + (0.15 if _event_type(a)==_event_type(b) else 0.0))


def _within_window(a: dict, b: dict, hours: int = 96) -> bool:
    ta, tb = _timestamp(a), _timestamp(b)
    minimum = datetime.min.replace(tzinfo=timezone.utc)
    return ta == minimum or tb == minimum or abs((ta-tb).total_seconds()) <= hours*3600
...
def _cluster(items: list[dict]) -> dict[str,list[dict]]:
    ...
    for item in sorted(items, key=_timestamp, reverse=True):
        ...
        if signature in groups:
            groups[signature].append(item); continue
        matched, best_score, anchor = None, 0.0, _entity_anchor(item)
        for rep_key, rep_item in representatives:
            if not _within_window(item, rep_item): continue
            ...
            if anchor_conflict and score < 0.72: continue
            accept = score >= 0.52 or (anchor_match and same_type and score >= 0.30)
            if accept and score > best_score: matched, best_score = rep_key, score
        if matched: groups[matched].append(item)
        else: groups[signature] = [item]; representatives.append((signature,item))
    return groups
...
def _score(items: list[dict]) -> dict:
    rows=[]
    for item in items:
        base=float(item.get("ai_score") or 0); text=_norm(" ".join([item.get("title",""),item.get("summary","")])) ; topic=item.get("research_topic")
        ...
        if _event_type(item)=="personnel": actionability=min(actionability,42); impact=min(impact,55)
    ...
    unique_domains=len({_domain(x) for x in items if _domain(x)}); independent_bonus=min(12,max(0,unique_domains-1)*5)
    confidence=min(100,sum(x[3] for x in rows)/len(rows)+independent_bonus); novelty=max(55,100-(len(items)-1)*12)
...
def _evidence_quality(items: list[dict], evidence: list[dict]) -> tuple[float,str]:
    ...
    # Source independence dominates: a perfectly traceable single article is
    # still materially weaker than two independent sources.
    coverage=round((source_diversity*.50+traceability*.25+date_verified*.25)*100,1)
    return coverage, ("cross_checked" if source_count>=2 else "single_source")


def _trust(coverage: float, conflict: bool=False) -> str:
    if conflict or coverage < 55: return "low"
...
def _insight(items: list[dict], scores: dict, event_type: str, entities: list[str]) -> dict:
    ...
    source_count=len({x.get("source_name") for x in items if x.get("source_name")}); evidence=_evidence(items); coverage,evidence_status=_evidence_quality(items,evidence); signals=extract_signals(title,summary,topic=lead.get("research_topic"))
    why=(f"该变化已由 {source_count} 个独立信源交叉报道，适合进入事件跟踪。" if source_count>1 else f"它涉及{topic}的关键变化，目前仍需独立证据确认。" if scores["impact"]>=75 else f"它属于{topic}的变化信号，目前更适合作为趋势观察，不宜仅凭单一报道下结论。")
    watch=("除非出现战略调整、组织重组或经营指标变化，否则建议降低持续关注优先级。" if event_type=="personnel" else "继续追踪后续公告、监管文件、市场数据和竞争对手动作，判断是否需要调整业务判断。" if scores["actionability"]>=70 else "观察是否出现第二个独立信源、监管动作或同类公司跟进，以确认趋势是否形成。")
...
def build(data: dict) -> dict:
    ...
    for event_id,items in _cluster(news).items():
        items=sorted(items,key=_timestamp,reverse=True); scores=_score(items); lead=items[0]; entities=sorted({e for item in items for e in _entities(item)})[:16]; event_type=_event_type(lead); evidence=_evidence(items); evidence_coverage,evidence_status=_evidence_quality(items,evidence); review_required=evidence_coverage<75 or event_type in {"regulatory","rating","claims_loss"}; conflict=False; trust_level=_trust(evidence_coverage,conflict)
    ...
    events.sort(key=lambda x:(x["scores"]["intelligence_score"],x.get("published_at") or ""),reverse=True); today=datetime.now(timezone.utc).date().isoformat(); daily=[e for e in events if (e.get("published_at") or "").startswith(today)][:5] or events[:5]; event_types=Counter(e["event_type"] for e in events)
    return {"version":MODEL_VERSION,"generated_at":datetime.now(timezone.utc).isoformat(),"principle":"发现值得行动的变化，而不是堆积更多新闻","model":{"article":"source item","event":"entity + action + topic + time","evidence":"traceable source support","decision":"advisory only / human approval boundary"},"events":events,"daily_brief":daily,"radar":build_radar(events),"stats":{"news_count":len(news),"event_count":len(events),"multi_source_events":sum(1 for e in events if e["source_count"]>1),"review_required_events":sum(1 for e in events if e["review_required"]),"event_types":dict(event_types)}}
