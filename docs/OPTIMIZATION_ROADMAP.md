...
Signal → Event → Claim → Evidence → Trust → Insight → Decision → Human Review → Release Provenance
...
| P1 | 发布可靠性 | Cloudflare 串行部署、release marker、deployment verification | 发布可验证且不因并发提交取消 |
...
- [ ] Claim/Evidence 纳入唯一主发布链
- [ ] Event → Claim → Evidence → Decision 页面闭环
- [ ] Claims benchmark 与反证指标
- [ ] Executive Decision View 正式主入口
- [ ] Knowledge Graph 第一版
