...
def build_radar(credibility=None, intelligence=None, impacts=None, backlog=None, review=None, trend_attribution=None) -> dict:
    ...
    return {'version':2,'generated_at':datetime.now(timezone.utc).isoformat(),'status':credibility_status,'principle':'雷达只排序已有风险信号；趋势归因只调整人工注意力，不重新评分、不修改原始决策、不自动执行行动。','items':candidates[:30],'summary':{'items':len(candidates),'top_attention_score':candidates[0]['attention_score'] if candidates else 0,'credibility_status':credibility_status,'impacted_event_count':len(impact_events)}}
