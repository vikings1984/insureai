...
class TestCounterfactual(unittest.TestCase):
    def test_conflict_guardrail_dependency_is_visible(self):
        data={'events':[{'event_id':'e1','topic':'regulatory_change','event_type':'regulatory','scores':{'intelligence_score':90},'trust':{'level':'medium','conflict':True}}],'temporal':{'topic_signals':[{'topic':'regulatory_change','phase':'accelerating','signal_strength':90}]}}
