...
PROFILE_VERSION = 1
ROLE_LABELS = {"executive":"管理层 / 高管","product":"产品","underwriting":"核保","actuarial":"精算","investment":"投资 / 资管","technology":"科技 / 数字化","claims":"理赔","distribution":"渠道"}
...
def personalize_event(event: dict, profile: dict | None) -> dict:
    ...
    role_event_types = {"executive":{"acquisition","capital","regulatory","market_entry"},"product":{"product","market_entry","regulatory"},"underwriting":{"rating","claims_loss","product","regulatory"},"actuarial":{"claims_loss","rating","capital","catastrophe"},"investment":{"capital","acquisition","rating","regulatory"},"technology":{"product","market_entry","regulatory"},"claims":{"claims_loss","product","regulatory"},"distribution":{"market_entry","product"}}
...
def personalize(events: list[dict], profile: dict | None) -> list[dict]:
    out = []
    for event in events:
        item = dict(event); item["personalization"] = personalize_event(event, profile); out.append(item)
