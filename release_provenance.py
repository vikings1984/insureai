...
# release-provenance-v1-compatible: add deployment identity binding without a breaking schema bump.
...
def attach_deployment_verification(*, root: Path = ROOT) -> dict:
    ...
    marker_matches = bool(expected_marker and deployment.get("release_marker") == expected_marker)
    verified = bool(deployment.get("verified", False)) and marker_matches
    deployment_status = "verified" if verified else ("stale" if deployment.get("verified") and not marker_matches else deployment.get("status", "failed"))
